 ...
 *
 * Usage:
 *   ADDIN_BASE_URL="https://dieuluucanh.github.io/pi-for-office" node scripts/generate-manifest.mjs
 *   ADDIN_BASE_URL="https://pi-for-office.example.com" OUT=manifest.prod.xml node scripts/generate-manifest.mjs
 *
...
function fail(msg) {
 console.error(`[pi-for-office] ${msg}`);
 process.exit(1);
}
...
if (!baseUrlRaw) {
 fail(
  'Missing ADDIN_BASE_URL. Example: ADDIN_BASE_URL="https://dieuluucanh.github.io/pi-for-office"',
 );
}
...
console.log(`[pi-for-office] Wrote ${outPath}`);
...
console.log(`[pi-for-office] Wrote ${publicOutPath}`);
