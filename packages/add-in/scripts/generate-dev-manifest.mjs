...
export function resolveDevOrigin({ arg, env = {} } = {}) {
 ...
 if (
  ...
  throw new Error(
   `Dev proxy origin must be a bare https origin without path/query/credentials (${source}): ${raw}`,
  );
 }
...
export function renderDevManifest(xml, origin) {
 if (!xml.includes(DEV_BASE_URL)) {
  throw new Error(
   `Input manifest does not include expected dev base URL ${DEV_BASE_URL}`,
  );
 }
...
function fail(message) {
 console.error(`[pi-for-office] ${message}`);
 process.exit(1);
}
...
if (isCurrentModuleEntrypoint()) {
 ...
 console.log(
  `[pi-for-office] Wrote ${outPath} (${resolved.origin}, from ${resolved.source})`,
 );
 console.log(
  `[pi-for-office] Validate with: node scripts/validate-manifest.mjs ${path.basename(outPath)}`,
 );
 console.log(
  `[pi-for-office] Sideload it the same way as manifest.xml — see docs/portless.md`,
 );
}
