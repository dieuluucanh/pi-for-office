...
Agent Skill for feature-specific validation of pi-for-office inside real China-domestic WPS Spreadsheets on a local Windows 11 ARM QEMU VM.
...
Separate **product-level Pi for Office proof** from **low-level WPS JSAPI probes**. Product-level proof must use the real Pi taskpane/sidebar, auth/model setup, agent loop, tools/approvals, workbook updates, and final user-visible result. Custom probe taskpanes are useful for isolating WPS APIs, but they are not proof that Pi for Office works for that feature.
