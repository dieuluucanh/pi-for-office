...
export function resolveDefaultProxyUrl(raw: DynamicValue): string {
 ...
 if (!/^https:\/\//i.test(candidate)) {
  console.warn(
   `[pi-for-office] Ignoring VITE_PI_DEFAULT_PROXY_URL (must be https://): ${candidate}`,
  );
 ...
 try {
  ...
  console.warn(
   `[pi-for-office] Ignoring VITE_PI_DEFAULT_PROXY_URL (not a valid URL): ${candidate}`,
  );
  return DEFAULT_LOCAL_PROXY_URL;
 }
...
export const DEFAULT_PROXY_URL = resolveDefaultProxyUrl(
 typeof import.meta.env === "undefined"
  ? undefined
  : import.meta.env.VITE_PI_DEFAULT_PROXY_URL,
);

/**
 * True when this build's default proxy is a remote (org/central) proxy.
 * UI copy uses this to swap local-helper instructions ("run npx pi-for-office-proxy")
...
export const PROXY_HEALTH_HEADER = "x-pi-for-office-proxy";
export const CODEX_WEBSOCKET_BRIDGE_HEADER =
 "x-pi-for-office-codex-websocket-bridge";

export const PROXY_HELPER_DOCS_URL =
 "https://github.com/dieuluucanh/pi-for-office/blob/main/packages/add-in/docs/install.md#oauth-logins-and-cors-proxy";
